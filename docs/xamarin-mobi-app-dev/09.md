...
文件链接的一个常见用途是用于小型或原型平台特定的应用，这些应用需要核心库的轻量级替代方案。您可以将所有的共享文件(业务逻辑、数据访问、实用程序等等)放入一个单独的项目(比如 Android 或 Xamarin。表单项目)，然后从其他特定于平台的项目(如 iOS 和/或 Windows Phone 项目)链接到这些文件。文件链接而不是核心库是一种快速而肮脏的方法，可能不应该在企业级或团队项目中使用。
...
在专业级 Xamarin 应用中，核心库对于保存您的 BLL、DAL、实用程序、接口和其他后端、非 UI、跨平台类非常有用。
