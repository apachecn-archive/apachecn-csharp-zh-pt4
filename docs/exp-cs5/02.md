...
本章研究了 C#对象以及它与内存堆栈和堆之间的关系。值类型的生命停留在堆栈中，而引用类型停留在堆中。我将使用`windbg.exe`程序解释这些，并讨论 CLR 执行任何内存时内存的不同部分。NET 应用程序。

最后，本章将通过在执行程序时检查堆栈和堆内存来讨论装箱和拆箱。

### 记忆在。网络应用

英寸 NET 中，当应用程序运行时，CLR 使用两种内存(堆栈和堆)来存储值类型和引用类型。CLR 使用堆栈内存来存储与方法相关的信息，这些信息在执行方法时被称为*方法状态*，它使用堆内存来存储应用程序范围的信息。在方法状态部分，CLR 在完成执行时存储局部变量、参数和方法返回地址。在堆内存中，CLR 存储应用程序和 Jitted 代码(由 Just in Time [JIT]编译的代码)使用的所有对象(大对象和小对象)。中执行托管应用程序时，CLR 为存储分配四个堆内存段。网络:
...
当 CLR 开始执行一个程序时，它会将内存分配给进程堆、JIT 代码堆、垃圾收集器(GC)堆和 LOH，这些内存被构造到系统、共享和默认应用程序域中。[图 2-1](#fig_2_1) 显示了执行. NET 应用程序时 CLR 分配的内存，如[清单 2-1](#list_2_1) 所示。
...
![images](images/9781430248606_Fig02-01.jpg)

***图 2-1。**记忆中的片段。网络应用*

在。NET 堆中，CLR 通过存储应用程序所需的引用类型的实例和应用程序的状态来管理一系列地址以跟踪程序的执行。在 C#类中，当您定义用于执行这些方法的方法时，CLR 存储用于处理其任务的局部变量、用于从调用方获取数据的参数(如果有)以及返回数据，即该方法向调用方的输出。它还需要保存返回执行点的信息(通过地址)。那个。NET 使用一种叫做*栈*的数据结构来跟踪所有这些信息。

![images](images/square.jpg)在本地运行时，通过`windbg.exe`提取的内存信息可能会有所不同。
...
![images](images/square.jpg) `clrstack`:在`windbg.exe`工具中使用，确定托管应用中的堆栈跟踪方法。

![images](images/9781430248606_Fig02-02.jpg)
...
![images](images/9781430248606_Fig02-03.jpg)
...
![images](images/9781430248606_Fig02-04.jpg)

***图 2-4。**栈和堆的组合显示了 CLR 如何在堆中存储一个对象*

稍后使用地址(`0x0184b64c`)从应用程序访问对象，例如从`Main`方法。[图 2-4](#fig_2_4) 演示了当 CLR 执行语句时
...
![images](images/square.jpg) `.ctor`:指 IL 中的构造函数。在探索中。克特和。cctor 使用第十五章的[部分，有关于`.ctor`和`.cctor`的讨论。](15.html#ch15)
...
![images](images/9781430248606_Fig02-05.jpg)
...
![images](images/9781430248606_Fig02-06.jpg)
...
![images](images/9781430248606_Fig02-07.jpg)
...
装箱和取消装箱是耗时的过程，可能会影响应用程序的性能。装箱操作比赋值操作多花 20 倍的时间。当性能是应用程序的关键因素时，正确使用装箱操作非常重要。让我们看看装箱和拆箱操作是如何影响应用程序的性能的。在清单 2-11 的[中，`int`的列表存储在`List<int>`类型的实例中。并且](#list_2_11)[清单 2-12](#list_2_12) 使用`ArrayList`进行存储。
...
![images](images/9781430248606_Fig02-08.jpg)
...
Gen 2 包含长寿命的对象。长期对象的一个例子是包含静态数据的服务器应用程序，这些数据在流程持续期间是活动的。
...
在本章中，我们已经了解了 CLR 在执行托管应用程序时的内存使用情况。我们研究了堆栈和堆，CLR 如何使用它们来存储应用程序中值类型和引用类型的数据，以及装箱和取消装箱操作的步骤。我们还看到了装箱和取消装箱操作如何影响应用程序的性能。在下一章，我们将学习。NET 设置值和引用类型参数，当您使用`out`和`ref keywords,`传递参数时会发生什么，以及 CLR 如何处理命名参数。